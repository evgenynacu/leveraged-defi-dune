-- Dune Query 5953061: RLP yield rates (Daily) - only burns
-- https://dune.com/queries/5953061
...
with day_bounds AS (
  SELECT dt as day_start,
    ...
    SELECT
        evt_block_time,
        POW(10, 12) * CAST(withdrawalAmount as DOUBLE) / CAST(burnedAmount as DOUBLE) as rate
    FROM resolv_ethereum.rlpmintandredeemrequests_evt_burnrequestcompleted
), data as (
    select rate,
           evt_block_time,
