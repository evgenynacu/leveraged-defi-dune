-- Dune Query 6256035: wsrUSD yield rates (Daily)
-- https://dune.com/queries/6256035
...
with day_bounds AS (
    ...
    select cast(assets as double) / cast(shares as double) as rate,
      ...
      from reservoir_protocol_ethereum.savingcoinv2_evt_deposit
), intervals as (
...
SELECT day_start AS ts_day,
       ...
       SUM(value * seconds_inside) / NULLIF(SUM(seconds_inside), 0) * 365 as yearly_yield_rate,
